<?php
/**
 * Элементарный транслитератор
 * Внимнаие! Хотя файл и в UTF-8, но он работает со строками в Windows-1251
 */
namespace Tools;

class PuntoSwitcherCp1251
{
	...
	/**
	 * Перекодировать русский ввод в латинский
	 *
	 * @param string $string
	 * @return  string
	 */
	public static function Ru2En ($string)
	{
		...
		return strtr ($string, self::$table);
	}

	/**
	 * Похожа ли строка на русскую
	 * Строка считается похожей на русскую если в ней нет латинских букв
	 *
	 * @param string $string
	 * @return bool
	 */
	public static function isLikeRu ($string)
	{
		$string = iconv ('windows-1251', 'utf-8', $string);

		$tmp = str_replace (self::$table, '', $string);
		return $string == $tmp;
	}
